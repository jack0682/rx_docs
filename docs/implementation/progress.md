...
## 2026-09-13 · 실제 두 컨테이너의 설치·승인·소재 처리·협력 종료

- P/S 제품 이미지를 사용한 새 설치 인수 도구를 추가했다. exporter는 공개 구성·정책·시험 신원만 만들고 Engine 행, session, Host ACK, Run, 자격을 주입하지 않는다. 실제 S 도구가 패키지를 조립·외부 서명·봉인·컴파일하며 실제 제품 P API가 반입·검토·구성 적용·재검증·독립 역할 승인·Host 자격 수용·활성화를 수행한다.
- 등록 단말 UI의 StartRun 응답을 commit 뒤 유실시키고 새로고침 후 동일 key/body로 회수했다. 한 Run의 서로 다른 소재2개와 작업2개가 완료·인계됐으며 P operation ID와 독립 FileDevice invocation/effects2회를 대조했다. desktop/mobile의 최신 완료 표시와 오류/CSP/가로 넘침을 확인했다. PHYSICAL 별도 셀은 NOT_COMMISSIONED이며 실제 시작을 거부한다. 모든 장비 동작은 FILE_SIMULATION이다.
- 새 `rx-service-status`는 같은 설치·역할·Host/셀·원장/설정·instance/PID에 묶인 64KiB 이하 원자 파일 보고를 제공한다. Linux BOOTTIME·최대2초 age·sequence·원문 digest·단독 writer를 검사한다. software heartbeat를 P 운전 허가나 물리 지지 증명으로 사용하지 않는다. 기존 INSTANCE_ID-only 호출은 유지하고 status 경로를 지정한 경우에는 불완전한 설정을 거부한다.
- S supervisor는 release-owned named Host/Executor recipe와 명시적 초기화 원장을 제공한다. 여러 Host/셀을 지원하고 동일 셀의 중복 Executor, principal/root 중복, definition 불일치와 필요한 Host dependency 누락을 기동 전에 거부한다. 초기화 Entered/Failed를 자동 재실행하지 않는다. guarded daemon은 자동 재시작과 강제 종료가 없으며 positive leader PID에만 TERM을 보내 planner/native child 정리는 각 owner가 담당한다.
- 실제 최종 배치는 P 하나·S 하나의 두 운전 컨테이너다. S의 status→Host→Executor 준비 순서, 작업 완료 뒤 동일 E instance/session의 Idle 대기, 관리자에 TERM 한 번을 보낸 뒤 E→H→status 종료를 확인했다. 각 guarded child의 exit0와 같은 scope/instance/PID/sequence/시각/원문 digest의 STOPPED가 일치해야 종료를 확인한다. P 종료 코드2와 남은 attention은 별도로 보존하며 깨끗한 전체 현장 종료라고 보고하지 않는다.
- 독립 검토에서 Host가 native adapter를 연 뒤 상태 보고 실패로 조기 반환할 수 있는 경계를 수정했다. safe_to_drop=false인 Held adapter를 유지하다 실제 허용 후 종료하는 두 Linux 실패 시점을 검증했다. 동일 셀에 서로 다른 E principal이 경쟁하는 구성도 차단했다. 최초 TERM 전송 실패를 성공처럼 기억하던 manager/initializer는 같은 owned child에 협력 요청을 재시도하도록 고치고 반례3개를 추가했다.
- 첫 supervised 인수에서는 두 소재 처리가 끝난 뒤 내부 RunService의 정상 Stopping을 상주 CellService 전체 종료로 잘못 보고했다. supervisor가 준비 상실을 감지해 요청 전에 전체를 종료했으므로 그 실행을 PASS로 세지 않았다. 전체 서비스와 내부 작업의 상태를 분리하고 Linux 반례4개를 통과한 새 이미지로 재실행했다. 최종 인수는 Idle 유지와 요청 종료까지 통과했다.
- 초기 독립 인수 도구의 권한 준비·Docker internal network 포트 노출·적용 전 ACK와 적용 후 proof 구별·정확한 change/runtime block 해제 집합의 문제를 수정했다. SIMULATION permit TTL은 실제 전달 경로의 회차 간 100ms 대기를 고려해 공개 envelope/profile에 1초로 명시했다. 조회 수명100ms 및 runtime gate는 바꾸지 않았으며 실물 시간 기준의 추천값으로 사용하지 않는다. 초기 첫 Prepare 거부를 이 TTL 결함만의 결과로 단정하지 않는다.
- 서명된 재검증 보고서·원문 artifact를 임시 저장소에만 두던 증거 보존 공백을 수정했다. 최종 공개 자료10묶음/56파일을 원문·파일별 hash로 다시 대조했다. 서명 seed·TLS private key·로그인 session을 증거로 반출하지 않는다. 서로 다른 역할 계정의 자동화를 실제 사람의 독립 평가로 표현하지 않는다. 복구 영역은 정확한 S 이미지의 봉인된 장애 시험을 참조하며 이번 사이클 자체가 전원 상실 시험을 수행했다고 주장하지 않는다.

최종 전체 Rust 시험은 P342개·S245개다. Linux supervisor27개, Host 보고 실패2개, 상태13개·E library34개 및 E 최종 CLI4개를 별도 검증했다(범위 간 중복 합산하지 않음). 양쪽 전체 정적 검사와 Linux 대상 검사, 최종 S 이미지의 기본 비운전 기동·필수42패키지/22profile·변조 정책 거부도 통과했다. UI37개 unit은 변경 없는 phase75 결과이며 이번 실제 제품 브라우저 인수와 구별한다. 규범8개·optional7개·SDK101개는 유지/동기화했다. [phase77 기록](../../references/implementation/phase77_checks.json)과 [최종 두 컨테이너 결과](../../references/implementation/phase77-cell-supervised-final/result.json)에 소스·이미지·실패 및 성공 증거를 연결한다. Host producer 재연결·운전 등록 rebind, 명시 복구/재개, 설치 교체·복원과 나머지 R01–R30은 계속 미완료다.

## 2026-09-13 · P 재시작 후 유휴 Host의 증거 세션 회복

- Publisher가 증거 없는 로컬 Idle tick만 반복하여 P 재시작을 감지하지 못하는 공백을 수정했다. 성공한 빈 원격 Publish의 monotonic 시각을 별도로 두고 유휴1초마다 실제 session/prefix를 확인한다. 로컬100ms tick과 실패는 이 시각을 갱신하지 않으며 기존 최대5초 backoff를 유지한다. Unauthenticated에만 고정 destination·store generation·Host boot/journal로 Session.Open/Cell.Open/prefix를 다시 수행한다.
- P는 실제 Host incarnation 변경과 P-only session 교체를 좁게 구분한다. Host boot·journal·auth binding·old active session/clock이 같고, 현재 P가 이전 session runtime을 철회한 정확한 immutable RuntimeRestart origin이 모든 등록 셀과 현재 구성/범위를 덮을 때만 추가 DeviceRestart를 생략한다. 기존 RuntimeRestart/AuthorityRevoked/다른 block, epoch, qualification, Run/mandate/permit/work, HostRegistration과 grant를 복원하지 않는다.
- source generation은 이 API 입력에 없으므로 source 연속성을 평가했다고 표현하지 않는다. 실제 source 변경의 observation/fact 무효화와 DeviceRestart, 과거 source에 묶인 등록은 그대로 보존한다. 정상/비정상 P stop, identity/session/origin/전체 셀·scope 반례, commit 실패/응답 유실·연속 restart와 기존 source 철회의 보존을 전용8개로 검증했다. 최초 실패는 fixture가 event 조회 상한128을 넘긴 것이며 bounded pagination으로 수정했다.
- 실제 제품 이미지의 별도 P/H 시험을 추가했다. 새 미자격 FILE_SIMULATION 셀을 제품 CLI로 설치하고 H를 유지한 채 P만 TERM→같은 DB/config로 run한다. 읽기 전용 SQLite transaction으로 Host producer/session/등록/cursor를 대조하며 제품 저장소를 직접 쓰지 않는다. stopped immutable 읽기는 실제 P exit 및 잔여 WAL 없음/0크기를 확인한 뒤에만 사용한다. Host status의 pending_evidence=null은0으로 해석하지 않고 실제 evidence 원장의 record 수를 별도 조회한다.
- 기존 phase77 이미지에서는 새 P의 로그인 성공 뒤에도 producer가 이전 P session에 머물러 대기 기한을 넘는 실패를 재현했다. 수정한 실제 이미지에서는 새 P session과 cell 협상, 이후 같은 session으로 빈 요청2회의 P cursor revision 증가·through0을 확인했다. 같은 H instance/boot/journal/descriptor, operating 등록 원문과 기존 제한을 보존하며 native effects/evidence0·Run0·자격 미복원을 대조했다. 정상 첫 P stop은0, rebind 미완료 이후 최종 P stop은2를 보존한다.
- 초기 harness 실패도 구분해 남긴다. 엄격한 Python TLS 검사에서 발견한 시험 인증서의 authority key identifier 누락은 fixture를 보완했고 인증서 검사를 끄지 않았다. pending_evidence의 null 의미와 stopped WAL 읽기 방식도 수정했다. 이 준비 실패를 제품 재연결 결함 재현으로 세지 않는다. 반복 probe는 단순 대기 대신 실제 P cursor commit 증가로 확인한다.
- 현재 빈 Publish도 P cursor revision과 감사 사건을 기록하므로 유휴1초 주기는 Host당 하루 약86,400회 기록이 가능하다. Native evidence가 증가하지 않는 것과 저장 비용이 없는 것은 다르다. 장기 보존·용량/부하와 probe 전달 최적화는 R17/R28의 미완료 범위로 남긴다.

전체 회귀 P350개·S247개 및 양쪽 전체 정적 검사가 통과했다. 최신 두 이미지로 phase77의 실제 설치/승인/UI StartRun 응답 유실 회수/소재2·독립 effects2/같은 E Idle/협력 종료를 다시 통과했다. 공개 자료56파일 hash와 private material 부재를 확인했으며 규범8개·optional7개·SDK101개를 유지한다. [phase78 기록](../../references/implementation/phase78_checks.json), [유휴 재연결 결과](../../references/implementation/phase78-idle-reconnect-final/result.json), [두 컨테이너 회귀](../../references/implementation/phase78-cell-delivery-regression/result.json). operating HostRegistration의 명시적 rebind, 활성 생산 작업의 복구/재개, 실제 source/Host 교체·물리 qualification과 나머지 R01–R30은 여전히 미완료다.

## 2026-09-13 · 관리자 승인에 따른 Host 복구 통신과 원래 미확정 작업 조회

- 최초 정상 pinned Host 연결의 transport/계약/배포·실제 configuration/source 읽기와 원래 P configuration·producer·최종 receipt를 불변 baseline 및 전체 원문 sidecar로 원자 보관한다. source 전체 hash/size/schema를 검증하고 retained source/receipt가 있는데 baseline만 없어진 경우 무결성 오류다. 과거 unpinned 등록에 근거를 소급 생성하지 않으며 renewal/current 등록을 역사 원문과 혼동하지 않는다.
- 같은 저장소 P-only restart 뒤 현재 ReleaseManager·등록 단말이 전체 영향 셀/CAS·runtime origin·기존 작업을 검토해 복구 연결을 제안·승인한다. 제품 worker는 배포 파일의 고정 transport registry와 실제 pinned read만 사용한다. 현재 신원/시간/source/configuration·원래 서명 Apply proof를 다시 검증하며, 관리자 요청으로 grant·Arm·새 operation admission을 수행하는 통로를 제공하지 않는다.
- 기존 runtime Fence가 정확히 하나라면 원래 outbox/request ID/body를 채택한다. 전송 진입과 outbox 상태는 원자 기록하며 같은 요청만 진행한다. ACK/늦은 관측을 사실로 보존하지만 현재 조건이 달라지면 Attention이며 RECOVERY_ONLY로 승격하지 않는다. 운영 HostRegistration·grant·제한·자격은 유지하고 알려진 기존 operation의 receipt/native lookup만 허용한다. 선택 operation의 evidence prefix를 전체 증거라고 표시하지 않으며 원래 Host publisher가 전체 순서를 전달한다.
- 실제 첫 이미지에서는 producer/session/publisher가 살아 있어도 최초 등록이 실패했다. 변경 시에만 출력하는 단계/typed reason·수치 진단을 추가해 PREPARE_LINK/ContinuityUnproven과 구성+source 조회164–176ms를 확인했다. custom pinned connector와 P/H incoming stream이 tonic 기본 TCP_NODELAY를 우회하고 있었다. client 수정만으로 약82ms와 실제 인수 성공, 양쪽 incoming 수정 후 약1.3ms를 관측했다. 100ms read 검증과 기존 계약을 완화하지 않았다. 로컬 모의 측정값을 현장 지연 상한으로 주장하지 않는다.
- 실제 두 이미지의 유휴 P-only 재시작/API 인수는 최초 baseline·새 producer와 원래 등록 보존, 역할 거부, 제안 전 Fence 미전송, 동일 제안/승인 회수, 정확한 원래 Fence ACK, progress와 임의 operation 거부를 통과했다. native effects/evidence/새 Run은0이다. 소스/clock/구성 변경·모호한 Fence·권한 철회·재로그인·VoidedBeforeSend·원문 누락/변조 등 core/API 반례도 별도 검증했다.
- 운영 화면은 실제 commit 뒤 제안 응답을 유실시키고 송신 전에 보관한 같은 raw body/key로 새로고침 후 회수했다. 승인·RECOVERY_ONLY·목록을 통한 재발견,1440/390폭·한글 font·외부 resource 없음과 제한 보존을 확인했다. 초기 실패는 selector exact label 가정과 실제 CSP 이벤트였다. 이벤트 원문 위치로 Zod의 동적 코드 capability probe를 확인하고 모든 schema를 jitless runtime으로 구성했다. 동적 Function을 금지한 반례와 최종 실제 브라우저에서 CSP/예상 밖 console/page error0을 확인했다. 정책을 약화시키지 않았다. receipt 없는 구성 조회를 현재 연결 실패로 오해하게 하는 문구도 정정했다.
- 미확정 작업 인수는 실제 P/E 제품 실행파일과 별도 S Host test fixture를 사용했다. fixture는 기존 FileDevice 효과 저장 뒤 반환만 감추고, 모의 결과 visibility marker 전/후 원래 lookup을 HIDDEN/FOUND로 분리했다. 실제 서명 commissioning/API 수량1 시작→effect1→P-only restart→명시 복구 승인→원래 조회→Host 원장1/publisher ACK/P 원문 evidence를 검증했다. 독립 SUBMIT/effects는1, 반복 RESULT_CAPTURED 조회는 native lookup/effect/evidence를 추가하지 않았다. 이 fixture 실행파일을 출하 rx-hostd 자체의 fault injection으로 주장하지 않는다.
- 위 공정은 native 성공 외에 후조건도 요구한다. 과거 permit과 현재 셀의 연속성이 끊겼으므로 성공 capture를 회수해도 Work는 RECONCILING/UNKNOWN/NONE/QUARANTINED와 conclusion evidence_ids=[]를 유지했다. 초기 시험의 SUCCEEDED/NOT_NEEDED 기대는 기존 규칙과 충돌하여 수정했다. release 계정이 다른 물리 셀을 볼 수 있다는 잘못된 시험 가정도 기존 operator 계정의 실제 재로그인으로 고쳤다. 권한 범위를 확장하지 않았다.
- E의 옛 session은 철회됐다. 정리 시 UNAUTHENTICATED와 원래 Run/session의 PENDING stop intent·ATTENTION·admission=false를 보존하고 exit1을 반환했다. 이 사실을 정상 종료로 바꾸지 않는다. H fixture exit0, P 전/후 exit2와 제한을 보존했다. E operating rebind/명시 재개와 전체 현장 정상 종료는 후속 범위다.

전체 Rust 회귀 P390개·S247개, 양쪽 전체 정적 검사, UI49개와 typecheck/build/format, Linux fixture build/정적 검사, 규범8개·optional binding7개·SDK101개가 통과했다. 최신 이미지로 기존 실제 두 컨테이너의 signed commissioning→UI StartRun 응답 유실 회수→소재2/effects2 완료·인계→같은 E Idle→supervisor 협력 종료도 통과했다(P exit2 잔여 attention 유지). [phase79 기록](../../references/implementation/phase79_checks.json)에 정확한 이미지/원문/hash·각 실패와 성공 범위를 연결한다. R01–R30 전체 완료와 실제 장비 지원은 주장하지 않으며 첫 물리 셀은 NOT_COMMISSIONED다.

## 2026-09-13 · 미완료 실행기 원장의 오프라인 점검

- S 제품 실행파일에 `cell recovery-inspect CONFIG`를 연결했다. 기존 서비스/설치/설정/소유 파일을 확인하고 원 assignment·creation marker·현재 attachment·run header·request/index/event history·stop을 검사한다. 네트워크/Session.Open/planner/native 호출, header 초기화, attachment 승격·종료, 원 request/stop 수정은 하지 않는다. 같은 원본은 새 시각/PID 없이 동일한 digest/JSON을 반환한다.
- 기존 RW/WAL/migration 저장소 열기를 재사용하지 않았다. 기존 service owner와 DB writer lock을 생성 없이 확보하고, 파일 종류·경로·크기·inode/시각·hash를 확인한 DB/WAL/SHM의 제한된 임시 사본을 읽는다. SQLite의 schema6/테이블·record/event 수·aggregate bytes·연속 history를 검사하며 write Transaction 메서드는 거부한다. Preparing/creation 미진입이면 없는 Run 파일을 만들지 않고, creation 이후 파일 상실은 명시 실패다.
- 첫 독립 감사에서 실제 P2를 찾았다. main에는 초기 빈 상태만 checkpoint하고 WAL에 Attached/EmitEntered/PENDING을 commit한 뒤 별도 사본의 WAL magic1byte를 훼손하면, SQLite가 WAL을 무시해 attachment=None/pending0 성공을 반환했다. 정상 no-SHM 사본의 정확한 미완료 회수는 통과했고 손상 거부 반례는 실패했다. 당시 소스 archive/hash와 실패 log를 보존했다.
- SQLite 전에 비어 있지 않은 WAL header·page size·양 endian 누적 checksum·frame salt/페이지 범위·완결 frame/commit 끝을 검증하도록 보완했다. 잘못된 header와 불명 tail에서 main-only 성공으로 후퇴하지 않는다. 원본 SHM의 digest는 보존하지만 사본 SHM은 버리고 검증 WAL에서 다시 구성하여 오래된 임시 index가 후속 commit을 가리지 않게 했다. stale-SHM 대조에서도 최신 Attached/PENDING 회수·두 번 동일 결과·원문/mtime 보존을 확인했다.
- 정상 SQLite가 허용하는 old-salt 재사용 tail·미완료/미commit tail도 첫 점검 범위에서는 WAL_TAIL_UNPROVEN으로 거부한다. 이를 곧 손상이라고 단정하지 않는다. WAL 없음/0바이트/header-only는 현재 frame 없음이며 독립적인 과거 commit watermark가 아니다. 전체 WAL 삭제/유효한 옛 prefix까지 절단/DB와 WAL 동시 rollback은 이 검사로 증명하지 못한다. [공식 형식과 판단 근거](../../references/implementation/phase80_sqlite_wal_notes.md)에 이 한계를 명시했다.
- 실제 P/E 제품 실행파일과 S native fault fixture의 기존 known-query 인수에 새 제품 CLI 점검을 연결했다. 실제 commissioning·수량1/native effect1·P-only restart·관리자 복구/원래 capture 회수와 UNKNOWN/격리 보존을 먼저 검증한 뒤, E ATTENTION/PENDING/exit1 원장을 network-none에서 두 번 읽었다. 출력 동일, 원본 E 파일/디렉토리8개의 bytes·mtime·mode·size 불변, 원 stop/session/attachment 보존, P/H 권한/outbox/Run/work 및 native trace 불변을 대조했다. 원 E 정리 성공이나 현재 P/물리 상태 확인으로 표시하지 않는다.
- Linux 실제 CLI15개(신규5개 포함), 로컬 원장5개, 실제 WAL 손상/stale-SHM2개와 정적 검사를 통과했다. checksum unit5개는 별도이며 여러 suite의 겹치는 시험을 합산하지 않는다. 최종 S 전체259개, 전체 clippy/format도 통과했다. 기존 P390개·UI49개는 변경 없는 phase79 근거이며 P 이미지 입력367개의 byte hash로 재사용을 확인했다. 규범8개·optional binding7개·SDK101개도 유지한다.
- 새 S 이미지로 기존 두 제품 컨테이너의 signed commissioning→UI 응답 유실 회수→소재2/effects2 완료·인계→같은 E Idle→supervisor 협력 종료도 통과했다. P exit2 잔여 attention, known-query의 E exit1/PENDING·H exit0·P2/2는 구별해 보존했다.

[phase80 기록](../../references/implementation/phase80_checks.json), [실제 오프라인 점검 결과](../../references/implementation/phase80-executor-recovery-inspect/executor-inspection-result.json), [정상 공정 회귀](../../references/implementation/phase80-cell-delivery-regression/result.json). 새 E 등록/collect, 원 stop 결과 회수·attachment retire, operating Host rebind/shared renewal, 첫 T5 조사 처분·검증된 자원 해제·명시 RestartRun은 아직 후속이다. [후속 순서](next_steps/phase80_recovery_sequence.md)에 현재 규범·읽기 검토·선택지를 분리했다. R01–R30 전체와 실제 장비 지원은 미완료이며 물리 셀은 NOT_COMMISSIONED다.
