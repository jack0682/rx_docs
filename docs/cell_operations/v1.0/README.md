...
상태: 설계 기준판 확정 · 2026-09-10 · 규범적 설계 문서. 실물 운전 허가·안전기능 성능·인증 판정은 아니다.

이 계약은 셀의 **운전 조건, 실행 허가, 작업자 개입, 복구, 변경 후 재검토**를 정의한다. [공통 작업 계약 v1.0](../../contracts/v1.0/README.md)에 결합하며 기존 작업의 접수·결과·불명·중복 처리 의미를 소급 변경하지 않는다.
...
| [조사와 결정 근거](../../../references/cell_operation_research_2026-09-10/research_and_decisions.md) | 원문 사실, RX 정책, 대안 선택·배제 이유 |
...
- Rust/ROS 경계·두 RX 레포/컨테이너·자사 필수 지원 범위는 유지한다. 새로운 필수 중앙 안전 서버나 세 번째 레포를 전제하지 않는다.

## 적용 범위

같은 주 컴퓨터의 단일 Runtime과 장비 Host, 첫 레이저 셀을 기준으로 한다. 고정식·바퀴형·족형 모델의 차이를 profile로 표현한다. 실제 로봇 선정·레이저 제어부·신호·개입 절차가 미확정인 첫 설치는 `NOT_COMMISSIONED`다. 이 상태에서도 계약 설계는 확정할 수 있지만 생산/현장 접근 허용을 발급할 수는 없다.
