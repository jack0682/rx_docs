...
규범: RX 셀 운영 계약 v1.0 · 원문 선택 근거는 [조사 보고서](../../../references/cell_operation_research_2026-09-10/research_and_decisions.md)의 S01–S13/O-D01–11을 따른다.
...
| 항목 | 현재 기록 | 실행 허용에 필요한 입력 |
|---|---|---|
| 셀 용도 | 네모엔지니어링 레이저 열처리 설비의 소재 공급 | 실제 시연/생산/교시 범위와 운용 절차 |
| 설비 PLC | Q03UDVCPU·QJ61BT11N·CC-Link 자료 확인 | PLC 프로그램·통신 설정·신호 의미·고장/재부팅 동작 |
| 레이저 가공부 | 제어부·PLC와의 관계 미확정 | 방사/열/가공 상태·진입/기동 조건·OEM 자동화 인터페이스 |
| 로봇/tool/지그 | OMY·UR5e·FR3 후보, 실제 선택/교정 미확정 | 설치 개체·모델·버전·tool·지그·좌표·하중 조합 |
| 작업 | 집기·공급·척·문·다음 소재 준비 언급 | 실제 순서/병행 범위, ‘빼기’의 후퇴/회수 의미 |
| 사람 | 개입 필요, 범위 미확정 | 개입 목적/위치·접근/정비·인수인계·재시작 절차 |
| 검증/인수 | 실물 안전기능·납품 범위 미확정 | 모델/환경별 검증 결과와 사용 한계 |

현재 첫 셀의 commissioning 상태는 `NOT_COMMISSIONED`다. 이는 현장 입력의 상태이며 소프트웨어 설계 결과의 실패가 아니다. 알 수 없는 값을 default true나 ‘해당 없음’으로 채우지 않는다. 가공 start의 담당을 RX로 자동 배정하지 않는다. [현장 근거](../../../references/laser_plc_interface.md), [제품 범위](../../03_product_scope.md).
...
`Qualification`은 envelope와 그 의존 artifact hash에 대해 검증 근거·검토 역할·제약·철회 정보를 기록한다. 상태는 `DRAFT / IN_REVIEW / QUALIFIED / SUSPENDED / RETIRED`다. QUALIFIED 전이는 필수 근거 연결과 검토 기록을 요구한다. 미충족 항목이 있으면 그 항목과 관련 없는 명시적 축소 envelope를 새로 정의·검증할 수 있지만 원래 요구 범위를 통과로 표시하지 않는다. 자사 필수 지원 항목은 삭제하지 않는다.
...
고정식 모델은 축/tool/소재 지지, 바퀴형은 위치·도킹·제동/주행과 팔의 결합, 능동 균형형은 전신 지지·반력·정지/회복 공간·에너지 조건을 추가한다. 같은 stop/Damping/zero velocity 명칭이 같은 물리 상태를 뜻하지 않는다. 각각의 자사 지원 ID는 13 지원표와 같은 model/mode 단위로 연결한다.
